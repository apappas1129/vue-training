...
import { Todo } from '#root/common/index';
...
export const useTodos = defineStore('todos', {
  ...
  getters: {
    todoById: state => (id: Todo['id']) => state.todoList.find((todo: Todo) => todo.id === id),
  },
  actions: {
    async fetchTodoList() {
      // simulate an API response
      const result = await new Promise<Todo[]>(resolve => setTimeout(() => resolve(todos), 250));
